...
import { NewStoryPhotoPicker } from '../components/NewStoryPhotoPicker'
...
interface StoriesViewProps {
  ...
  onCreateStory: (title: string, groups: StopGroup[]) => Promise<void>
}
...
function storyMeta(story: StoryWithStops): string {
  const photos = story.stops.reduce((sum, s) => sum + s.storyPhotos.length, 0)
  const notes = story.stops.reduce((sum, s) => sum + s.storyPhotos.filter((sp) => sp.answer).length, 0)
...
export function StoriesView({
  ...
  function openPicker(visitIds?: string[]) {
    ...
    <div className="h-full overflow-y-auto bg-story-cream font-story-sans text-story-ink">
        ...
        <div className="mx-auto flex max-w-lg flex-col gap-3 px-5 pb-28 sm:px-6">
          {stories.length === 0 && (
            <p className="py-6 text-center text-[13px] text-story-faint">
              No stories yet — switch to Holidays to start one.
            </p>
          )}
          {stories.map((story) => {
            const coverStop = story.stops.find((s) => s.storyPhotos.length > 0)
            const cover = coverStop?.storyPhotos[0]?.photo
            ...
            return (
              ...
              >
                {cover ? (
                  <PhotoThumb storagePath={cover.storage_path} className="h-[82px] w-[82px] flex-shrink-0 rounded-[14px] object-cover" />
                ) : (
      ...
      {tab === 'holidays' && (
        <div className="mx-auto flex max-w-lg flex-col gap-3 px-5 pb-28 sm:px-6">
          {suggestions.length === 0 && (
            <p className="py-6 text-center text-[13px] text-story-faint">
              No new holidays spotted yet — log a few visits close together and they'll show up here.
            </p>
          )}
          {suggestions.length > 0 && (
          ...
          {suggestions.map((cluster) => (
            ...
            >
              ...
              <button
                onClick={() => openPicker(cluster.visitIds)}
      ...
      {showPicker && (
        <NewStoryPhotoPicker
          ...
          userEmail={userEmail}
          onCreate={async (title, groups) => {
            await onCreateStory(title, groups)
            setShowPicker(false)
          }}
